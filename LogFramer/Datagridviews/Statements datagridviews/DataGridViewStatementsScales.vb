Public Class DataGridViewStatementsScales
                ...
                Select Case .QuestionType
                    Case Indicator.QuestionTypes.Scale
                        For intIndex = 0 To .Statements.Count - 1
                            Me.Grid.Add(New StatementGridRow(.Statements(intIndex), .ResponseClasses(intIndex)))
                        Next
                    Case Indicator.QuestionTypes.CumulativeScale
                        For intIndex = 0 To .Statements.Count - 1
                            Me.Grid.Add(New StatementGridRow(.Statements(intIndex), .ResponseClasses(0)))
                        Next
                End Select
            End With
        End If
...
#Region "Cell images"
    ...
    Private Sub ReloadImage_Response(ByVal selRow As StatementGridRow)
...
#Region "General methods"
    Private Sub VerifyStatementResponseClassCount()
        With CurrentIndicator
            If .Statements.Count > .ResponseClasses.Count And .QuestionType <> Indicator.QuestionTypes.CumulativeScale Then
                For i = .ResponseClasses.Count + 1 To .Statements.Count
                    .ResponseClasses.Add(New ResponseClass())
                Next
            ElseIf .ResponseClasses.Count > .Statements.Count Then
                For i = .Statements.Count + 1 To .ResponseClasses.Count
    ...
    Public Sub InsertItem()
        ...
        If CurrentCell IsNot Nothing Then
            ...
            If CurrentIndicator.QuestionType <> Indicator.QuestionTypes.CumulativeScale Then
                Dim NewResponseClass As New ResponseClass
                CurrentIndicator.ResponseClasses.Insert(intIndex, NewResponseClass)
                UndoRedo.ItemInserted(NewResponseClass, CurrentIndicator.ResponseClasses)
            End If
    ...
    Public Sub MoveItem(ByVal intDirection As Integer)
        ...
        Dim selResponseClass As ResponseClass = CurrentIndicator.ResponseClasses(intIndex)
        ...
        If CurrentIndicator.QuestionType <> Indicator.QuestionTypes.CumulativeScale Then
            CurrentIndicator.ResponseClasses.Remove(selResponseClass)
            CurrentIndicator.ResponseClasses.Insert(intIndex, selResponseClass)
        End If

        If intDirection < 0 Then
            UndoRedo.ItemMovedUp(selStatement, selStatement, CurrentIndicator.Statements, intOldIndex, CurrentIndicator.Statements)
            UndoRedo.ItemMovedUp(selResponseClass, selResponseClass, CurrentIndicator.ResponseClasses, intOldIndex, CurrentIndicator.ResponseClasses)
        Else
            UndoRedo.ItemMovedDown(selStatement, selStatement, CurrentIndicator.Statements, intOldIndex, CurrentIndicator.Statements)
            UndoRedo.ItemMovedDown(selResponseClass, selResponseClass, CurrentIndicator.ResponseClasses, intOldIndex, CurrentIndicator.ResponseClasses)
        End If
    ...
    Public Sub SetFocusOnItem(ByVal selItem As Object)
        ...
        Select Case selItem.GetType
            ...
            Case GetType(ResponseClass)
                Dim selResponseClass As ResponseClass = DirectCast(selItem, ResponseClass)

                intColIndex = 1
                intRowIndex = CurrentIndicator.ResponseClasses.IndexOf(selResponseClass)
        End Select
    ...
    Public Overrides Sub RemoveItems(ByVal ShowWarning As Boolean, Optional ByVal boolCut As Boolean = False)
        ...
        If Me.IsCurrentCellInEditMode = False Then
            ...
            For Each selGridRow As StatementGridRow In SelectedGridRows
                ...
                selResponseClass = selGridRow.ResponseClass
                ...
                If CurrentIndicator.QuestionType <> Indicator.QuestionTypes.CumulativeScale Then
                    If boolCut = False Then
                        UndoRedo.ItemRemoved(selResponseClass, CurrentIndicator.ResponseClasses)
                    Else
                        UndoRedo.ItemCut(selResponseClass, CurrentIndicator.ResponseClasses)
                    End If
...
#Region "Copy and paste cells"
                ...
                For i = .FirstRowIndex To .LastRowIndex
                    ...
                    For j = .FirstColumnIndex To .LastColumnIndex
                        If j = 0 Then
                            ...
                            End If
                        ElseIf j = 1 Then
                            If ItemClipboard.Count = 0 OrElse ItemClipboard(0).Item IsNot selRow.ResponseClass Then
                                strSort = ResponseClass.ItemName
                                Dim NewItem As New ClipboardItem(CopyGroup, selRow.ResponseClass, strSort)
                                ItemClipboard.Insert(0, NewItem)
                            End If
                        End If
                    Next
    ...
    Public Overrides Sub PasteItems(ByVal PasteItems As ClipboardItems, ByVal intPasteOption As Integer, Optional ByVal PasteCell As DataGridViewCell = Nothing)
        ...
        For i = 0 To PasteItems.Count - 1
            ...
            Select Case selItem.Item.GetType
                ...
                Case GetType(ResponseClass)
                    PasteItems_ResponseClass(selItem, intPasteOption)
                Case Else
    ...
    Private Sub PasteStatement(ByVal NewStatement As Statement)
        Dim PasteRowResponse As Statement = PasteRow.Statement
        Dim intIndex As Integer = CurrentIndicator.Statements.IndexOf(PasteRowResponse)
    ...
    Private Sub PasteItems_ResponseClass(ByVal selItem As ClipboardItem, ByVal intPasteOption As Integer)
        ...
        If CurrentIndicator.QuestionType <> Indicator.QuestionTypes.CumulativeScale Then
            Dim NewStatementClass As New ResponseClass

            Using copier As New ObjectCopy
                NewStatementClass = copier.CopyObject(selResponseClass)
            End Using

            PasteResponseClass(NewStatementClass)
        End If
    End Sub

    Private Sub PasteResponseClass(ByVal NewResponseClass As ResponseClass)
        Dim PasteRowResponseClass As ResponseClass = PasteRow.ResponseClass
        Dim intIndex As Integer = CurrentIndicator.ResponseClasses.IndexOf(PasteRowResponseClass)

        If intIndex = -1 Then intIndex = CurrentIndicator.ResponseClasses.Count

        CurrentIndicator.ResponseClasses.Insert(intIndex, NewResponseClass)

        PasteRow.ResponseClass = NewResponseClass

        UndoRedo.ItemPasted(NewResponseClass, CurrentIndicator.ResponseClasses)
    End Sub
    ...
    Private Sub PasteItems_Other(ByVal selItem As ClipboardItem, ByVal intPasteOption As Integer)
        ...
        If String.IsNullOrEmpty(strRtf) = False Then
            ...
            PasteStatement(NewStatement)
        ElseIf CurrentIndicator.QuestionType <> Indicator.QuestionTypes.CumulativeScale Then
            Dim NewStatementClass As New ResponseClass(LANG_Agree)
            Double.TryParse(strText, NewStatementClass.Value)
            PasteResponseClass(NewStatementClass)
        End If
    End Sub
#End Region
...
#Region "Virtual mode"
    ...
    Protected Overrides Sub OnCellValuePushed(ByVal e As System.Windows.Forms.DataGridViewCellValueEventArgs)
        ...
        If selRowIndex < Me.Grid.Count Then
            ...
            If Me.EditRow Is Nothing Then
                ...
                With EditRow
                    ...
                    .ResponseClass = CurrentGridRow.ResponseClass
        ...
        Select Case strColName
            Case "Statement"
                If RowTmp.Statement Is Nothing Then RowTmp = SetNewRow(RowTmp)

                RowTmp.FirstLabel = strCellValue
                RowTmp.StatementImageDirty = True
            Case "Value"
                If RowTmp.ResponseClass Is Nothing Then RowTmp = SetNewRow(RowTmp)

                UndoRedo.UndoBuffer_Initialise(RowTmp.ResponseClass, "Value", RowTmp.ResponseClass.Value)

                Dim selValue As Double
                Double.TryParse(e.Value, selValue)

                RowTmp.Value = selValue
                UndoRedo.ValueChanged(selValue)
        End Select

        'CurrentUndoList.AddToUndoList()
    End Sub

    Private Function SetNewRow(ByVal selGridRow As StatementGridRow) As StatementGridRow
        If selGridRow.Statement Is Nothing Then
            Dim NewStatement As New Statement
            CurrentIndicator.Statements.Add(NewStatement)
            selGridRow.Statement = NewStatement

            UndoRedo.ItemInserted(NewStatement, CurrentIndicator.Statements)
        End If

        If selGridRow.ResponseClass Is Nothing Then
            Dim NewResponseClass As New ResponseClass
            CurrentIndicator.ResponseClasses.Add(NewResponseClass)
            selGridRow.ResponseClass = NewResponseClass

            UndoRedo.ItemInserted(NewResponseClass, CurrentIndicator.ResponseClasses)
        End If

        If CurrentIndicator.QuestionType = Indicator.QuestionTypes.CumulativeScale Then
            selGridRow.ResponseClass = CurrentIndicator.ResponseClasses(0)
        End If
        Return selGridRow
    End Function
...
#Region "Custom painting - general"
    Protected Overrides Sub OnCellPainting(ByVal e As System.Windows.Forms.DataGridViewCellPaintingEventArgs)
        If Me.Grid.Count > 0 Then
            ...
            If Me.TextSelectionIndex = TextSelectionValues.SelectAll And e.ColumnIndex >= 0 And e.RowIndex >= 0 Then
            ...
            Else
                CellPainting_NormalText(e.FormattedValue, False, True, rCell, CellGraphics)
            End If
